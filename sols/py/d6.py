...
class Field:
    def __init__(self, content: Iterable, shape: tuple[int, int]):
        ...
        self.data: Any = [[i] for i in content]
    ...
    def set_square(self, dim0_idx: int, dim1_idx: int, value: Any) -> None:
...
def find_loops(raw_string, shape, direction: str, guard_pos: Vec) -> int:
    field = Field(raw_string, shape=shape)
    path = run_sim(field, direction, guard_pos)
    count = 0
    while len(path) > 0:
        field = Field(raw_string, shape=shape)
...
if __name__ == "__main__":
    ...
    # Part 1
    field: Field = Field(raw_string, shape=(ii, jj))
    ...
    start = time_ns()
    run("cnt = find_loops(raw_string, shape=(ii,jj), direction='NN', guard_pos=Vec(*guard_pos))", sort='cumtime')
